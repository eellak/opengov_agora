...
public abstract class DecisionGenericService<T extends IPublicOrganizationDecision> implements IDecisionGenericService<T> {
...
//	public abstract String getXmlPath();
	...
	public List<IDecisionStorageReference> save(List<T> decisions, IValidation validation ){
		if (decisions.size() > accessControl.getClient().getMaximumDecisionsWrite()) {
			throw new TooManyDecisionsToStoreException("Maximum for client: "
					+ accessControl.getClient().getMaximumDecisionsWrite());
		}
		
		for ( int i = 0; i < decisions.size(); i++ ) {
			T decision = decisions.get( i );
			checkReferences( decision );
			
			Map<IAda,IPublicOrganizationDecision> relatedDecisions = getRelatedDecisions(decision);
			
			IValidation cv = validator.validateCreate( decision, relatedDecisions );
			if ( !cv.isValid() ) {
				cv.addLocationPrefix(getXmlPath() + "[" + i + "]/" );
//				cv.addLocationPrefix( 
//										((ParameterizedType) getClass().getGenericSuperclass()).getActualTypeArguments()[0] + "s/" + 
//										((ParameterizedType) getClass().getGenericSuperclass()).getActualTypeArguments()[0] + "[" + i + "]/" );
				validation.addValidationErrors( cv );
			}					
		}
		logger.debug( "Validation: " + validation.isValid() );
		if ( !validation.isValid() ) return new ArrayList<IDecisionStorageReference>();
		return dao.save( decisions );
	}
	...
	public IDecisionStorageReference delete( String id, String reason, String deletionType ) {
		logger.debug( "Deleting decision: " + id );
		accessControl.checkAccess( OdeAccessControl.Operation.DELETE, get(id) );		
		logger.debug( "Validation was ok." );		
		return dao.delete(id, reason, deletionType, accessControl.getClient().getUserId() );		
	}
	...
	public void purge( String id ) {
		logger.debug( "Purge decision: " + id );
		accessControl.checkAccess( OdeAccessControl.Operation.PURGE, get(id) );	
		logger.debug( "Validation was ok." );				
		dao.purge(id);
	}
	...
	public IDecisionStorageReference update(T decision, IValidation validation ) {
		logger.debug( "Updating decision: " + decision.getId() );
		
		Map<IAda,IPublicOrganizationDecision> relatedDecisions = getRelatedDecisions(decision);
		
		IValidation cv = validator.validateUpdate( decision, relatedDecisions );
		if ( !cv.isValid() ) {
			cv.addLocationPrefix( ((ParameterizedType) getClass().getGenericSuperclass()).getActualTypeArguments()[0] + ": " + decision.getId() );
			validation.addValidationErrors( cv );
		}							
		logger.debug( "Validation: " + validation.isValid() );
		if ( validation.isValid() ){		
			accessControl.checkAccess( OdeAccessControl.Operation.UPDATE, decision );
			logger.debug( "Validation was ok." );
...
//	@Override
//	public boolean existsWithUniqueDocumentCode(String uniqueDocumentCode){
//		List<ISearchParameter> searchParams = new ArrayList<ISearchParameter>();
//		searchParams.add(new StringSearchParameter("uniqueDocumentCode", uniqueDocumentCode));
//		addGetDecisionsFilters(searchParams);
//		List<T> decisions = getAll( new PaginationInfo(), searchParams);
//		return decisions.size() != 0;
//	}
