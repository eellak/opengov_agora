...
$this->menu=array(
	array('label'=>Yii::t('payments','Update Payment'), 
		  'url'=>array('updatepayment', 'documentUrl'=>$model->documentUrl),
	...
	array('label'=>Yii::t('form','Cancel'),  
		...
		&& $model->cancelled=='false' && Yii::app()->user->role!='user',
		 'linkOptions'=>array('submit'=>array('cancelpayment','uniqueDocumentCode'=>$model->uniqueDocumentCode))),
	array('label'=>Yii::t('payments','Delete Payment'),  
		 'url'=>'#', 'linkOptions'=>array('submit'=>array('deletepayment','uniqueDocumentCode'=>$model->documentUrl)),
		 'visible'=>$model->actionPermitted && Yii::app()->user->role!='user' && $model->cancelled=='false'),
	...
	array('label'=>Yii::t('form','SuperSearch'), 'url'=>array('searchpayments'))
		...
		$this->renderPartial('/commons/TabCanceled',
				array('model'=>'payment','organizations'=>false,'cancelled'=>$model->cancelled,
	     ...
	     if($model->procurements){
	      ...
	      if($ContractData->notice[0]){
			...
			if(isset($model->protocolNumberCode) && $model->protocolNumberCode!=''){
				echo"<div class='row-grey'>";
				echo "<span class='title'>". Yii::t('payments','protocolNumberCode')."</span>";
	...
	<?php 
		...
		if(isset($ContractData->TotalItemcost[$num])){
			...
			echo "". Yii::t('item','TotalItempayment') .": &nbsp;".$ContractData->TotalItemcost[$num]."&nbsp;";
			echo"</div>";
		}
		...
		if(isset($ContractData->responsibilityAssumptionCode[$i]) && $ContractData->responsibilityAssumptionCode[$i]!=''){
				echo"<div class='row-grey'>";
				echo "<span class='title'>". Yii::t('procurement','responsibilityAssumptionCode')."</span>";
				echo "<span class='data'>". $ContractData->responsibilityAssumptionCode[$i]."</span>";
				echo"</div>";
		}
